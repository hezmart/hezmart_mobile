        ...
        child: BlocConsumer<OrderBloc, OrderState>(
          ...
          builder: (context, state) {
            ...
            if (state is GetAllOrderSuccesState) {
              final validStatuses = ['closed', 'cancelled'];

              final pendingDelivered =
                  state.reponse.data?.orders
                      ?.where(
                        (order) => validStatuses.contains(
                          order.status?.toLowerCase() ?? '',
                        ),
                      )
                      .toList();

              logger.w(pendingDelivered);
                    ...
                    (onggoingContext) => InkWell(
                      onTap: () {
                        context.pushNamed(
                          ...
                          queryParameters: {
                            PathParam.id:
                                pendingDelivered[onggoingContext]
                                    .items
                                    ?.first
                                    .orderId
                                    .toString() ??
                            ...
                            Expanded(
                              child: Column(
                                ...
                                children: [
                                  ...
                                  TextView(
                                    text:
                                        "Order: ${pendingDelivered[onggoingContext].orderNumber ?? ''}",
                                  ),
                                  10.verticalSpace,
